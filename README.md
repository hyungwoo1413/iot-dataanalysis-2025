...
- 지도 학습(supervised learn) - 데이터 -> `입력`, 정답 -> `타겟` => 훈련 데이터(training data)
    - 입력 - 특성
    - 입력과 타겟을 모두 주어서 훈련
- 비지도 학습(unsupervised learn) - 입력만 존재하고 타겟 없이 훈련
- 강화 학습(reinforcement learn) - 선택가능한 행동 중 보상과 처벌 등으로 최적의 행동양식을 학습
...
### 빅데이터에 필요한 모듈
- Matplotlib(차트), Seaborn(차트꾸미기), Numpy(배열), Pandas(데이터 조작)
...
- `과대적합` - overfit : 훈련세트 성능이 테스트세트 성능보다 훨씬 높을때
