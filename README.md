...
- `하이퍼 파라미터` - 머신러닝, 딥러닝에서 학습하지 않는 파라미터
    - 사람이 직접 지정하는 값
...
- SGD 설명 정리
    - 모든 데이터를 가지고 손실함수 최소값을 찾는 것이 아닌
    - 데이터를 일정 간격으로 뛰어넘으면서 손실함수 최소값을 찾는 횟수를 줄이는 것
    - 방향을 살짝 틀어서 다음에는 덜 틀리게 만드는 방법
